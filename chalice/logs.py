...
"""
from typing import IO, Any, Dict, Iterator, Optional  # noqa
...
def display_logs(retriever, max_entries,
                 include_lambda_messages, stream, follow):
    # type: (LogRetriever, int, bool, IO[str], bool) -> None
    events = retriever.retrieve_logs(
        include_lambda_messages=include_lambda_messages,
        max_entries=max_entries, follow=follow)
    for event in events:
        stream.write('%s %s %s\n' % (
            event['timestamp'],
            event['logShortId'],
...
class LogRetriever(object):
    ...
    def retrieve_logs(self,
                      include_lambda_messages=True,  # type: bool
                      max_entries=None,              # type: Optional[int]
                      follow=False                   # type: Optional[bool]
                      ):
        ...
        log_events = self._client.iter_log_events
        if follow:
            log_events = self._client.follow_log_events

        shown = 0
        for event in log_events(self._log_group_name, interleaved=True):
