...
class LambdaDeploymentPackager(BaseLambdaDeploymentPackager):
    def create_deployment_package(
        ...
        package_filename = self.deployment_package_filename(
            project_dir, python_version, architecture=architecture
        )
...
class DependencyBuilder(object):
    ...
    def _is_compatible_wheel_filename(
        self, expected_abi: str, filename: str, architecture: str = 'x86_64',
    ) -> bool:
        wheel = filename[:-4]
    ...
    def _is_compatible_platform_tag(
        self, expected_abi: str, platform: str, architecture: str = 'x86_64',
    ) -> bool:
        ...
        compatible_platforms = {'any', 'linux_%s' % arch}
        if platform in compatible_platforms:
    ...
    def _download_binary_wheels(
        ...
        self._pip.download_manylinux_wheels(
            ...
            directory,
            self._get_pip_platforms(abi, architecture=architecture),
        )
    ...
    def _get_pip_platforms(
        ...
        platforms = [
            'manylinux_%s_%s_%s' % (runtime_major, minor, arch)
            for minor in range(17, runtime_minor + 1)
        ]
        platforms.append('manylinux2014_%s' % arch)
        return platforms
    ...
    def _categorize_wheel_files(
        self, abi: str, directory: str, architecture: str = 'x86_64',
    ...
    def _download_dependencies(
            ...
            architecture: str = 'x86_64') -> Tuple[Set[Package], Set[Package]]:
        ...
        sdists, compatible_wheels, incompatible_wheels = self._categorize_deps(
            abi, deps, architecture=architecture
        )
        ...
        compatible_wheels, incompatible_wheels = self._categorize_wheel_files(
            abi, directory, architecture=architecture
        )
