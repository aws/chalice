...
from chalice.utils import OSUtils, serialize_to_yaml
...
ROLE_TEST_CASES = [
    ...
    RoleTestCase(
        ...
        given={
            ...
            'g': {'autogen_policy': False, 'iam_policy_file': 'g.yml'},
            'h': {'autogen_policy': False, 'iam_policy_file': 'h.yml'}
        },
        roles={
...
class TestPolicyGeneratorStage(object):
    ...
    def test_policy_loaded_from_file_if_needed(self):
        ...
        self.osutils.get_buffered_contents.assert_called_with('foo.json')

    def test_error_raised_if_file_policy_not_exists(self):
        p = self.create_policy_generator()
        policy = models.FileBasedIAMPolicy(
            filename='foo.yml', document=models.Placeholder.BUILD_STAGE)
        self.osutils.get_buffered_contents.side_effect = IOError()
...
class TestRecordResults(object):
    ...
    def test_can_record_results_initial_deploy(self):
        expected_filename = os.path.join(self.deployed_dir, 'dev.yml')
        ...
        expected_contents = serialize_to_yaml(self.deployed_values)
