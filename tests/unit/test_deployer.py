...
def test_vpc_config_no_subnet(sample_app):
    config = Config.create(
        chalice_app=sample_app, vpc_config={'subnet_ids': []}
    )
    with pytest.raises(ValueError):
        validate_configuration(config)
...
def test_apig_methods(stubbed_session):
    ...
    apig.create_method_request(
        'resource_id', 'GET', url_params=['name'],
        authorization_type='TEST', authorizer_id='TED',
...
def test_policy_autogenerated_when_enabled(app_policy,
                                           in_memory_osutils):
    cfg = Config.create(
        autogen_policy=True, project_dir='.'
    )
...
def test_policy_autogenerated_when_enabled_with_vpc(app_policy,
                                           in_memory_osutils):
    cfg = Config.create(
        autogen_policy=True, project_dir='.',
        vpc_config={'subnet_ids': ['12345']}
    )
    in_memory_osutils.filemap['./app.py'] = ''
    generated = app_policy.generate_policy_from_app_source(cfg)
    # We don't actually need to validate the exact policy, we'll just
